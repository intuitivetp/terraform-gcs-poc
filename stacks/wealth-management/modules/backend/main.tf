...
resource "google_service_account" "backend" {
  account_id   = "${var.environment}-wealth-api"
  display_name = "Wealth API Service Account"
  description  = "Service account for ${var.environment} wealth API"
}

resource "google_cloud_run_service" "api" {
  name     = "${var.environment}-wealth-api"
  location = var.region
