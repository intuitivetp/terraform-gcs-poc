...
resource "google_storage_bucket" "frontend" {
  name          = "${var.project_id}-${var.environment}-wealth-frontend"
  location      = var.region
...
resource "google_storage_bucket_object" "index" {
  ...
  content = <<-EOT
    ...
    <html>
      <head>
        <title>Wealth Management - ${var.environment}</title>
        <style>
      ...
      <body>
        <div class="header">
          <h1>🏦 Wealth Management Portal</h1>
          <p>Environment: ${upper(var.environment)}</p>
        </div>
        <div class="content">
          <h2>Welcome to Wealth Management</h2>
          <p>This is a demo wealth application deployed on GCP.</p>
