...
resource "google_monitoring_dashboard" "wealth_dashboard" {
  dashboard_json = jsonencode({
    displayName = "${var.environment}-wealth-dashboard"
    mosaicLayout = {
      columns = 12
      tiles = [
        {
...
resource "google_logging_project_sink" "wealth_logs" {
  name        = "${var.environment}-wealth-logs-sink"
  destination = "logging.googleapis.com/projects/${var.project_id}/locations/global/buckets/_Default"
  filter      = "labels.application=\"wealth-management\" AND labels.environment=\"${var.environment}\""
